		...
		<div class="card-body">
					...
					<div class="row">
						<div class="col-7">
							<div class="form-group">
								<div class="row">
									<div class="col-2">
										<label for="dose_name" class="font-weight-bold font-italic">Dose Name:</label>
									</div><!-- .col-4 end -->
									<div class="col-10">
										<input type="text" class="form-control" id="dose_name" name="dose_name" placeholder="Enter Dose Name">
									</div><!-- .col-8 end -->
								</div><!-- .row end -->
							</div><!-- .form-group end -->
						</div><!-- .col-5 end -->
			...
			<div class="outdoor-shift-list">
				<table class="table table-striped">
					...
					<tbody>
						@foreach(App\Models\Does::getAllDose() as $dose)
						<tr>
							<td>{{$loop->index+1}}</td>
							<td>{{$dose->dose_name}}</td>
							<td>{{$dose->status == 1 ? 'Active' : 'Inactive'}}</td>
							<td>
								<a href="{{route('dose.edit', $dose->id)}}"><i class="ion-edit"></i></a>
								<form class="d-inline" action="{{route('dose.destroy', $dose->id)}}" method="post">
									@csrf
